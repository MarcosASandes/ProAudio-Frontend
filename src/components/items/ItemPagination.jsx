    ...
    <nav aria-label="Page navigation">
      <ul className="pagination justify-content-center my-4">
        ...
        {Array.from({ length: total_pages }).map((_, idx) => (
          <li
            key={idx}
            className={`page-item ${page_number === idx ? "active" : ""}`}
          >
            <button className="page-link" onClick={() => onPageChange(idx)}>
              {idx + 1}
            </button>
          </li>
        ))}
...
const ItemPagination = ({ pageable, onPageChange }) => {
  ...
  const { page_number, total_pages, has_previous, has_next } = pageable;
  const currentPageOneBased = page_number + 1; // Ajustamos base 0 a base 1

  const handlePrevious = () => {
    if (has_previous) onPageChange(currentPageOneBased - 1); // va a página base 1 - 1 = base 0 para backend
  };
