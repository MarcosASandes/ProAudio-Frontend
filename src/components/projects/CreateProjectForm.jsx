...
import useGetStartingProjectStatus from "../../hooks/projects/useGetStartingProjectStatus";
import { useSelector } from "react-redux";
import { selectProjectTypes, selectStartingProjectStatus } from "../../features/projects/ProjectSelector";
import useGetExpenseTypes from "../../hooks/expenses/useGetExpensesTypes";
import { selectExpenseTypes } from "../../features/expenses/ExpenseSelector";
import ProductFieldItem from "./ProductFieldItem";
import { getProjectTypeLabel, getProjectStatusLabel, getExpensesTypesLabel } from "../../utils/getLabels";
...
const CreateProjectForm = () => {
  ...
  const handleGoToCreateEvent = () => {
    ...
    try {
      const cleaned = JSON.parse(JSON.stringify(currentData));
      localStorage.setItem("projectDraft", JSON.stringify(cleaned));
      navigate("/events/create/embedded", { state: { from: "create-project" } });
    } catch (err) {
      console.error("No se pudo guardar el draft:", err.message);
    }
  };
  ...
  const onSubmit = async (data) => {
    const cleanedProducts = data?.products?.map(({ product_id, price_id, amount }) => ({
      product_id,
      price_id,
      amount,
    }));
  ...
  return (
    <>
      <div className="mb-3">
        <button type="button" className={styles.btnBackArrow} onClick={handleGoBack}>
          <ArrowLeft size={24} />
          <span className="ms-2">Volver</span>
        </button>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className={styles.form}>
        ...
        <div className={styles.formGroup}>
          <label htmlFor="txtNameProject">Nombre del proyecto</label>
          <input id="txtNameProject" type="text" className="form-control" {...register("name")} />
        </div>

        <div className={styles.formGroup}>
          <label htmlFor="txtDescriptionProject">Descripción</label>
          <textarea id="txtDescriptionProject" rows="3" className={`form-control ${styles.textarea}`} {...register("description")} />
        </div>
        ...
        <div className="row mt-1">
          <div className="col-md">
            <div className={styles.formGroup}>
              <label htmlFor="dateStartProject">Fecha de inicio</label>
              <input id="dateStartProject" type="date" className="form-control" {...register("start_date")} />
            </div>
          </div>
          <div className="col-md">
            <div className={styles.formGroup}>
              <label htmlFor="dateFinishProject">Fecha de fin</label>
              <input id="dateFinishProject" type="date" className="form-control" {...register("end_date")} />
            </div>
          </div>
          <div className="col-md">
            <div className={styles.formGroup}>
              <label htmlFor="slcProjectType">Tipo de proyecto</label>
              <select id="slcProjectType" className="form-control" {...register("project_type")}>
                <option value="">Seleccionar tipo</option>
                {projectTypes?.map((type) => (
                  <option key={type} value={type}>
                    {getProjectTypeLabel(type)}
          ...
          <div className="col-md">
            <div className={styles.formGroup}>
              ...
              {selectedEvent ? (
                ...
                <div className="d-flex gap-2">
                  <button type="button" className="btn btn-outline-primary" onClick={() => setShowEventModal(true)}>
                    Seleccionar evento
                  </button>
                  <button type="button" className="btn btn-outline-success" onClick={handleGoToCreateEvent}>
                    Crear nuevo evento
                  </button>
                </div>
              )}
            ...
            <div className={styles.formGroup}>
              ...
              {selectedClient ? (
                <div className={styles.selectedClientBox}>
                  ...
                  <p className="m-0">{selectedClient.email}</p>
                  <button
                    type="button"
                    className="btn btn-sm btn-outline-danger mt-2"
                    onClick={() => {
                ...
                <div className="d-flex gap-2">
                  <button type="button" className="btn btn-outline-primary" onClick={() => setShowClientModal(true)}>
                    Seleccionar cliente
                  </button>
                  <button
                    type="button"
                    className="btn btn-outline-success"
                    onClick={() => toast.info("Funcionalidad de crear cliente aún no implementada")}
                  >
                    Crear cliente
                  </button>
                </div>
        ...
        <div className="mb-3">
          ...
          >
            <span>Productos</span>
            {isProductsOpen ? <ChevronDown size={18} /> : <ChevronRight size={18} />}
          </button>

          <div className={`collapse mt-2 ${styles.collapseBody}`} id="collapseProducts">
            {productFields?.map((field, index) => (
              <ProductFieldItem
                key={field.id}
                index={index}
        ...
        <div className="mb-3">
          ...
          >
            <span>Gastos extra</span>
            {isExpensesOpen ? <ChevronDown size={18} /> : <ChevronRight size={18} />}
          </button>

          <div className={`collapse mt-2 ${styles.collapseBody}`} id="collapseExpenses">
            {expenseFields?.map((field, index) => (
              <div key={field.id} className="border p-3 mb-2 rounded bg-secondary bg-opacity-10">
                <div className="mb-2">
                  <label className="form-label text-light">Tipo</label>
                  <select className="form-select" {...register(`expenses.${index}.type`)}>
                    <option value="">Seleccionar tipo de gasto</option>
                    {expensesTypes?.map((expType) => (
                      <option key={expType} value={expType}>
                        {getExpensesTypesLabel(expType)}
                ...
                <div className="mb-2">
                  <label className="form-label text-light">Valor</label>
                  <input type="number" onWheel={(e) => e.target.blur()} className="form-control" {...register(`expenses.${index}.value`)} />
                </div>
                <div className="mb-2">
                  <label className="form-label text-light">Descripción</label>
                  <input type="text" className="form-control" {...register(`expenses.${index}.description`)} />
                </div>
                <button type="button" className="btn btn-danger btn-sm" onClick={() => removeExpense(index)}>
                  Eliminar gasto
                </button>
              </div>
            ))}
        ...
        {errorMessages.length > 0 && (
          <div className="alert alert-danger">
            <strong>Se han detectado errores en el formulario:</strong>
            <ul className="mb-0">{errorMessages.map((msg, idx) => <li key={idx}>{msg}</li>)}</ul>
          </div>
        )}
              ...
              <div className="modal-body">
                <ProductSelectorModal
                  onSelect={(product) => {
                    ...
                    setValue(`products.${currentProductIndex}.product_label`, `${product.brand} ${product.model}`);
                    setShowProductModal(false);
                  }}
                />
              </div>
