...
const UpdateProjectForm = () => {
  ...
  useEffect(() => {
    ...
    // Base: si hay savedDraft lo uso, sino project
    const baseData = savedDraft || project;

    // Forzar que las fechas SIEMPRE se formateen
    const initialData = {
      ...baseData,
...
const UpdateProjectForm = () => {
  ...
  useEffect(() => {
    ...
    // leer temporales (si usas namespaced también)
    ...
    // marcar que ya hidrataron el formulario para que watch pueda guardar
    hydratedRef.current = true;
  }, [project, reset, id]);

  useEffect(() => {
    if (!id) return; // sin id no guardamos

    const subscription = watch((value) => {
      // Evitamos salvar antes de la hidratación inicial (para no pisar el draft leído)
    ...
    return () => {
      try {
        ...
        // por si la API cambia, evitar romper
      }
    };
  }, [watch, id]);
