...
const activeTags = [
  { id: 1, name: "Sennheiser" },
  { id: 2, name: "Micrófono" },
  { id: 3, name: "Inalámbrico" },
];
...
const ProductsTagsFilter = () => {
  return (
    <div className="tags-filter-wrapper d-flex align-items-center justify-content-between w-100">
      ...
      <div className="tags-filter-container d-flex align-items-center gap-2 flex-grow-1">
        {activeTags.map((tag) => (
          <span key={tag.id} className="badge tag-badge">
            {tag.name} <span className="remove-tag">✕</span>
          </span>
        ))}
      </div>

   
      <button className="btn btn-tag-add d-flex align-items-center justify-content-center ms-2">
        <Plus size={18} />
      </button>
    </div>
  );
