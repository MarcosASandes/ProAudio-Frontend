...
export const productSchema = Yup.object().shape({
  ...
  replacement_value: Yup.number()
    ...
    .max(99999999.99, 'El máximo valor permitido para el valor de reposición es: 99999999.99')
    .test(
      'max-decimals',
    ...
    .of(
      Yup.object().shape({
        value: Yup.number()
          ...
          .max(99999999.99, 'Máximo valor permitido para el precio es: 99999999.99')
          .test(
            'max-decimals',
            'Máximo dos decimales',
            value => /^\d+(\.\d{1,2})?$/.test(value?.toString() || '')
