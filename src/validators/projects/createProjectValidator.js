...
const createProjectValidator = yup.object().shape({
  ...
  event: yup.object().shape({
    ...
    address: yup
      .string()
      .max(100, "La dirección del evento no puede tener más de 100 caracteres")
  ...
  event: yup
    ...
    .test(
      ...
      function (value) {
        ...
        // Si no hay event_id y no hay event, falla
        if (!event_id && !value) return false;
        return true;
      }
    ),
  ...
  client: createClientValidator
    ...
    .test(
      ...
      function (value) {
        ...
        if (!client_id && !value) return false;
        return true;
      }
    ),
});
