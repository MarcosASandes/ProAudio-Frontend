...
/*export const getCreateProjectFormErrorMessages = (errors) => {
  ...
  if (errors.event) {
    ...
    if (errors.event.description) messages.push(errors.event.description.message);
  }
  ...
  if (errors.products) {
    errors.products.forEach((prod, index) => {
      if (!prod) return;
      if (prod.product_id) messages.push(`Producto ${index + 1}: ${prod.product_id.message}`);
      if (prod.amount) messages.push(`Producto ${index + 1}: ${prod.amount.message}`);
    });
  }
  ...
  if (errors.expenses) {
    errors.expenses.forEach((exp, index) => {
      ...
      if (exp.description) messages.push(`Gasto ${index + 1}: ${exp.description.message}`);
    });
  }
